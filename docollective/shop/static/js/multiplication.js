...
let button2 = document.getElementById('hid_btn2');
let input = document.getElementById('hid_inp');
let btn_valider = document.getElementById('hid_btn_valider');
let div1 = document.getElementById('hid_div1');

/*const Input = document.getElementById('Hid_inp');*/
let enregistrer = document.getElementById('hid_enregistrer');
let commentaire = document.getElementById("hid_commentaire");
let demarrer = document.getElementById("hid_demarrer");
...
function stop(){
    //ON ARRETE LE TEMPS ET ON LE STOCK
    clearInterval(timer);
    //localStorage.setItem("temps",String(t));
    taille = tableau.push(t);
    sum = tableau.reduce((bcc, v) => bcc + v);
    tempsMoyen = (sum/taille)/10 ;
    tempsMoyenCentieme = tempsMoyen.toFixed(2);
    document.getElementById('html_tempsMoyenCentieme').setAttribute('value', tempsMoyenCentieme); //ENVOI DANS L INPUT HIDDEN
...
function maz() {
    let input2 = document.getElementById('hid_inp');
    if (input.value=="") {input.value=0;}
    if (input2.value=="") {input2.value=0;}
}

/*function envoiDuTempsMoyen() {
    document.addEventListener("htmx:beforeOnLoad", function (event) {
        const XHR = new XMLHttpRequest();
        XHR.open("POST", "accounts/addresult.html");
        XHR.send(html_tempsMoyenCentieme.value);
    }
}*/
...
//btn_valider.addEventListener('click',maz ; stop)
//button2.addEventListener('click',activeContinuer ; changerCouleurEtFocus ; demarrerTime ; raz );
